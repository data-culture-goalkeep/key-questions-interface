  ...
  const [{ data: areas }, { data: keyQuestions }, { data: indicatorLevels }] =
    await Promise.all([
      ...
      supabase
        .from("key_questions")
        .select(
          "id, project_id, area_of_enquiry_id, kq_number, question_text, indicator_level_id, indicator_definition, action_text, primary_user, data_availability, priority, reason_for_priority, sequence, is_locked"
        )
        .eq("project_id", project.id)
        .order("sequence"),
      supabase
