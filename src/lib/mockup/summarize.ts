...
/**
 * Collate every comment flagged "to be incorporated" and ask Claude for a
 * next-steps table. Returns a discriminated result rather than throwing, so a
 * misconfiguration (missing key, no flagged comments, API error) surfaces a
 * readable message even in the production build (thrown server-action errors
...
export async function summarizeNextSteps(): Promise<SummarizeResult> {
  try {
    ...
    if (!apiKey)
      return { error: "The Anthropic API key isn't configured on the server." }

    const supabase = createAdminClient()

    const [
      { data: commentRows, error: cErr },
      { data: answerRows, error: aErr },
      { data: reviewers },
    ] = await Promise.all([
      supabase
      ...
      supabase.from("sandipani_reviewers").select("id, name"),
    ])
    if (cErr) return { error: `Database error: ${cErr.message}` }
    if (aErr) return { error: `Database error: ${aErr.message}` }
    ...
    for (const a of (answerRows ?? []) as {
      ...
      const rec =
        votesByEl.get(key) ??
        ({ yes: 0, partly: 0, no: 0 } as Record<AnswerValue, number>)
      rec[a.verdict]++
    ...
    try {
      ...
      if (e instanceof Anthropic.APIError) {
        if (/workspace/i.test(e.message) && !workspaceId)
          return {
            error:
              "The Anthropic API key isn't scoped to a workspace — set ANTHROPIC_WORKSPACE_ID or use a workspace-scoped key.",
          }
        return { error: `Anthropic API error (${e.status}): ${e.message}` }
      }
      return {
        error: e instanceof Error ? e.message : "The Claude request failed.",
      }
    }
    ...
    const content = response.content
      ...
      .join("\n")
      .trim()
    if (!content) return { error: "Claude returned an empty summary." }
    ...
    const { data: saved, error: sErr } = await supabase
      ...
      .single()
    if (sErr) return { error: `Couldn't save the summary: ${sErr.message}` }
