...
- `vercel.json` pins the Vercel function region to `hnd1` (Tokyo), matching the Supabase project's actual region (`ap-northeast-1`) — there was no `vercel.json` before, so functions defaulted to `iad1` (Washington, D.C.), a needless cross-region hop on every DB call inside every Server Action and dynamic page load. A live measurement (a temporary diagnostic route, since removed) showed a single trivial query dropping from ~405ms to ~80ms after the pin. Since the Supabase project is shared across other Goalkeep apps (not dedicated to this one), its own region isn't something this repo can change unilaterally — if real users turn out to be predominantly India-based (early signal: requests consistently enter via Vercel's Mumbai edge PoP) rather than Japan-based, moving the *database* to `ap-south-1` would need coordinating whoever else depends on that shared project, since Supabase has no in-place region change — only migrating to a new project.
